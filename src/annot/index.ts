...
/**
 * 将PDF批注导出到Excel文件
 * @param PDFViewerApplication PDF查看器应用程序实例
...
async function exportAnnotationsToExcel(PDFViewerApplication: PDFViewerApplication, annotations: IAnnotationStore[]) {
    const rows: any[] = [] // 用于存储Excel行数据的数组
    // 先按页码升序，再按批注时间降序排序批注
    annotations.sort((a, b) => {
        if (a.pageNumber !== b.pageNumber) {
            return a.pageNumber - b.pageNumber // 页码不同时按页码升序排列
        }
        return getPDFDateTimestamp(b.date) - getPDFDateTimestamp(a.date) // 页码相同时按批注时间降序排列
    })
    /**
     * 获取批注的最后状态名称
    ...
    const getLastStatusName = (annotation: IAnnotationStore): string => {
        ...
        const status = lastWithStatus?.status ?? CommentStatus.None // 如果没有状态则使用None
        return t(`comment.status.${status.toLowerCase()}`) // 返回翻译后的状态名称
    }

    let mainIndex = 1 // 主批注序号，从1开始计数
    let replyCounter: number = 0 // 回复计数器（每次主批注开始重置）

    // 遍历所有批注，构建Excel行数据
    annotations.forEach(annotation => {
        const annotationName = annotationDefinitions.find(def => def.type === annotation.type)?.name // 获取批注类型名称
        const typeLabel = t(`annotations.${annotationName}`) // 获取翻译后的批注类型标签
        // 主批注行数据
        rows.push({
            index: `${mainIndex}`, // 主批注序号
            id: annotation.id, // 批注ID
            page: annotation.pageNumber, // 页码
            annotationType: typeLabel, // 批注类型
            recordType: t('export.recordType.annotation'), // 记录类型（批注）
            author: annotation.title, // 作者
            content: annotation.contentsObj?.text || '', // 内容
            date: formatPDFDate(annotation.date, true), // 日期
            status: getLastStatusName(annotation) // 状态
        })
        // 重置回复计数器
        replyCounter = 0
        // 回复行数据
        annotation.comments.forEach(comment => {
            replyCounter++
            rows.push({
                index: `${mainIndex}.${replyCounter}`, // 主批注序号.回复序号
                id: comment.id,
                page: '', // 回复不显示页码
                annotationType: '--', // 回复不显示批注类型
                recordType: t('export.recordType.reply'), // 记录类型（回复）
                author: comment.title, // 作者
                content: comment.content, // 内容
                date: formatPDFDate(comment.date, true), // 日期
                status: '' // 回复不显示状态
            })
        })
        mainIndex++
