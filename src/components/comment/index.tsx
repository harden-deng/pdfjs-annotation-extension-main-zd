...
const CustomComment = forwardRef<CustomCommentRef, CustomCommentProps>(function CustomComment(props, ref) {
    ...
    const allTypes = useMemo(() => {
        ...
        console.log('allTypes--111->', Array.from(types.entries()))
        console.log('allTypes--2222->', types)
    ...
    useEffect(() => {
        setSelectedTypes(allTypes.map(([t],index) => {
            if(index === 0 || t === 'Highlight') {
                return t
            }
        }))
    }, [allTypes])
    ...
    const handleTypeToggle = (type: PdfjsAnnotationSubtype) => {
        console.log('点击过滤类型type--->', type)
        setSelectedTypes(prev => (prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type]));
        console.log('点击过滤类型type--->2222-->', selectedTypes)
    }
