...
export class Painter {
    ...
    public readonly onWebSelectionSelected: (range: Range) => void // Web 选中回调
    public readonly onStoreAdd: (annotationStore: IAnnotationStore, isOriginal: boolean, currentAnnotation: IAnnotationType) => void // 添加存储回调
    public readonly onStoreDelete: (id: string) => void // 删除存储回调
    public readonly onAnnotationSelected: (annotationStore: IAnnotationStore, isClick: boolean, selectorRect: IRect) => void // 选中批注回调
    public readonly onAnnotationChange: (annotationStore: IAnnotationStore) => void // 批注更改回调
    public readonly onAnnotationChanging: (id: string,isClick: boolean) => void // 批注正在更改的回调函数
    ...
    constructor({
        ...
        onAnnotationChanging: (id: string,isClick: boolean) => void
        ...
        this.selector = new Selector({
            ...
            onCancel: (id,isClick) => {
                console.log('选择----4-------->',id)
                this.onAnnotationChanging(id,isClick) // 批注正在更改的回调
            },
            onDelete: id => {
                console.log('选择----5-------->',id)
    ...
    private enableEditor({ konvaStage, pageNumber, annotation }: { konvaStage: Konva.Stage; pageNumber: number; annotation: IAnnotationType }): void {
        const storeEditor = this.findEditor(pageNumber, annotation.type) // 查找存储中的编辑器实例
         console.log('启用特定类型的编辑器----1-------->',storeEditor,pageNumber, annotation.type)
        if (storeEditor) {
    ...
    public activate(annotation: IAnnotationType | null, dataTransfer: string | null): void {
        ...
        // }
        switch (annotation.type) {
            case AnnotationType.SEE:
                this.setMode('default') // 设置绘画模式-浏览模式
                break
            case AnnotationType.SELECT:
                this.setMode('painting') // 设置默认模式-批注模式
                break
            default:
                this.setMode('default') // 设置绘画模式-浏览模式
                break
